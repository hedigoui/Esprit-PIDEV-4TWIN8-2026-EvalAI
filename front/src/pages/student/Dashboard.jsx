...
const defaultSkills = [
  { nameKey: 'fluency', score: 0, color: '#E31837' },
  { nameKey: 'pronunciation', score: 0, color: '#f97316' },
  { nameKey: 'speakingPace', score: 0, color: '#22c55e' },
  { nameKey: 'confidence', score: 0, color: '#3b82f6' },
  { nameKey: 'contentStructure', score: 0, color: '#8b5cf6' },
];
...
const Dashboard = () => {
  ...
  const images = useMemo(() => [
    { src: '/images/s1.jpg', alt: t('student.imgAltPractice') },
    { src: '/images/s2.jpg', alt: t('student.imgAltOral') },
    { src: '/images/eva.jpg', alt: t('student.imgAltBrand') },
  ], [t]);
  ...
  const skillRows = useMemo(() => {
    ...
    return [
      { nameKey: 'fluency', score: Math.round(sm.fluency ?? 0), color: '#E31837' },
      { nameKey: 'pronunciation', score: Math.round(sm.pronunciation ?? 0), color: '#f97316' },
      { nameKey: 'speakingPace', score: Math.round(pace), color: '#22c55e' },
      { nameKey: 'confidence', score: Math.round(sm.confidence ?? 0), color: '#3b82f6' },
      {
        nameKey: 'contentStructure',
      ...
      .map((r, i) => {
        ...
        return {
          ...
          date: r.performance?.createdAt
            ...
            : '—',
          title: r.performance?.title || t('student.sessionDefaultTitle'),
          score: sc,
          level,
  ...
  const aiRecs = useMemo(() => {
    ...
    if (!ca) {
      return [
        { icon: '🎉', title: t('student.recKeepPracticing'), desc: t('student.recKeepPracticingDesc'), bg: 'rgba(227,24,55,0.04)', accent: '#E31837' },
      ];
    }
    const out = [];
    if (ca.strengths?.length) {
      out.push({ icon: '💪', title: t('student.recStrength'), desc: ca.strengths[0], bg: 'rgba(34,197,94,0.06)', accent: '#22c55e' });
    }
    if (ca.improvements?.length) {
      out.push({ icon: '🎯', title: t('student.recFocusNext'), desc: ca.improvements[0], bg: 'rgba(249,115,22,0.06)', accent: '#f97316' });
    }
    if (ca.summary) {
      out.push({ icon: '🎯', title: t('student.recSummary'), desc: ca.summary.slice(0, 160), bg: 'rgba(59,130,246,0.06)', accent: '#3b82f6' });
    }
    return out.length ? out : [{ icon: '🚀', title: t('student.recGreatJob'), desc: t('student.recGreatJobDesc'), bg: 'rgba(227,24,55,0.04)', accent: '#E31837' }];
  }, [rows, t]);
        ...
        <main className={styles.content}>
          <div style={{ marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.15rem' }}>
              <span style={{ fontSize: '0.72rem', fontWeight: '600', color: '#E31837', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{t('student.dashboardBadge')}</span>
              <span style={{ width: '4px', height: '4px', borderRadius: '50%', background: '#22c55e', display: 'inline-block' }} />
              <span style={{ fontSize: '0.72rem', color: '#94a3b8' }}>{t('common.online')}</span>
            </div>
            <p style={{ color: '#94a3b8', fontSize: '0.85rem', marginTop: '0.3rem' }}>{t('student.oralSkillsSubtitle')}</p>
          </div>
          ...
          <div style={{
            ...
            <div style={{ position: 'absolute', bottom: '1.2rem', left: '1.5rem', right: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', zIndex: 10 }}>
              <div>
                <span style={{ display: 'inline-block', padding: '0.25rem 0.65rem', background: 'rgba(227,24,55,0.85)', borderRadius: '6px', fontSize: '0.65rem', fontWeight: '700', color: '#fff', marginBottom: '0.4rem', letterSpacing: '0.05em' }}>{t('student.imgAltBrand')}</span>
                <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.82rem', fontWeight: '500' }}>{t('student.heroTagline')}</p>
              </div>
          ...
          {loading && (
            <p style={{ color: '#94a3b8', fontSize: '0.85rem', marginBottom: '1rem' }}>{t('student.loadingStats')}</p>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
            <div style={{
              ...
              <div style={{ fontSize: '0.72rem', opacity: 0.75, marginTop: '0.2rem', fontWeight: '500' }}>{t('student.cefrLatest')}</div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.72rem', color: '#94a3b8', marginTop: '0.2rem', fontWeight: '500' }}>{t('student.overallAvg')}</div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.72rem', color: '#94a3b8', marginTop: '0.2rem', fontWeight: '500' }}>{t('student.sessionsCompleted')}</div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.72rem', color: '#94a3b8', marginTop: '0.2rem', fontWeight: '500' }}>{t('student.bestScoreLabel')}</div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '1rem', marginBottom: '1.5rem' }}>
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <div>
                  <h3 style={{ fontSize: '0.95rem', fontWeight: '700', color: '#1a1a2e', letterSpacing: '-0.01em' }}>{t('student.progressOverTime')}</h3>
                  <p style={{ fontSize: '0.72rem', color: '#94a3b8', marginTop: '0.15rem' }}>
                    {t('student.progressChartHint')}
                  </p>
                </div>
                {growthLabel && (
              ...
              {progressData.length === 0 ? (
                <p style={{ color: '#94a3b8', fontSize: '0.85rem', padding: '2rem 0', textAlign: 'center' }}>{t('student.completePracticeTrend')}</p>
              ) : (
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={progressData}>
                    ...
                    <Line type="monotone" dataKey="score" name={t('student.chartSessionScore')} stroke="#E31837" strokeWidth={2.5} connectNulls
                      dot={{ fill: '#fff', stroke: '#E31837', strokeWidth: 2, r: 3 }}
                      activeDot={{ fill: '#E31837', stroke: '#fff', strokeWidth: 2, r: 5 }}
                    />
                    <Line type="monotone" dataKey="holistic" name={t('student.chartHolisticIndex')} stroke="#3b82f6" strokeWidth={2} connectNulls strokeDasharray="6 4"
            ...
            <div style={{
              ...
              <h3 style={{ fontSize: '0.95rem', fontWeight: '700', color: '#1a1a2e', marginBottom: '1.2rem', letterSpacing: '-0.01em' }}>{t('student.skillsBreakdown')}</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {skillRows.map((skill) => (
                  <div key={skill.nameKey}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.3rem' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: '500', color: '#64748b' }}>{t(`student.${skill.nameKey}`)}</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <h3 style={{ fontSize: '0.95rem', fontWeight: '700', color: '#1a1a2e' }}>{t('student.recentEvaluationsTitle')}</h3>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {recentEvaluations.length === 0 ? (
                  <p style={{ color: '#94a3b8', fontSize: '0.82rem' }}>{t('student.noGradedSessions')}</p>
                ) : (
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                ...
                <h3 style={{ fontSize: '0.95rem', fontWeight: '700', color: '#1a1a2e' }}>{t('student.aiRecommendations')}</h3>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {aiRecs.map((rec, i) => (
                  <div key={i} style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div>
                <p style={{ color: '#fff', fontSize: '0.88rem', fontWeight: '600' }}>{t('student.readyToPractice')}</p>
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.72rem' }}>{t('student.readyToPracticeSub')}</p>
              </div>
            </div>
            <button type="button" onClick={() => navigate('/student/practice')} style={{
              ...
              {t('student.startPractice')} <ArrowUpRight size={15} />
            </button>
          </div>
        </main>
      </div>
