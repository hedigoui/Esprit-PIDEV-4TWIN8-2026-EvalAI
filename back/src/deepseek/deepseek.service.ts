...
export class DeepSeekService {
  ...
  async evaluateContent(
    ...
    try {
      ...
      if (error.response) {
        this.logger.error(`DeepSeek API error: ${JSON.stringify(error.response.data)}`);
      }
  ...
  private buildPrompt(
    ...
    const scoringGuide = mode === 'encouraging' 
...
Return ONLY this JSON:
{
  ...
  private fallbackEvaluation(transcript: string, subject: string): DeepSeekEvaluationResult {
    ...
    const hasRelevantTerms = subject.toLowerCase().split(/\s+/)
      .some(term => term.length > 3 && transcript.toLowerCase().includes(term));
    ...
    const hasEducation = /bachelor|degree|study|student|university|college/.test(transcriptLower);
    const hasHobbies = /hobby|like|love|enjoy|playing|watching|listening|reading/.test(transcriptLower);
    const hasFamily = /father|mother|brother|sister|parent|family/.test(transcriptLower);
    
    const elementCount = [hasName, hasOrigin, hasEducation, hasHobbies, hasFamily].filter(Boolean).length;
    
    // Determine if this is a self-introduction
    const isSelfIntroduction = subject.toLowerCase().includes('introduce') || 
    ...
    return {
      ...
      analysis: {
        ...
        strengths: [
          ...
          elementCount > 2 ? 'Covers multiple aspects of the topic' : 'Provides basic information',
      ...
      detailedFeedback: {
        structure: sentences.length > 3 ? 'Good organization with multiple sentences' : 'Try to organize your thoughts more clearly',
        contentGaps: ['Could include more specific examples relevant to the topic'],
        vocabularySuggestions: ['Try using more descriptive words', 'Use synonyms to avoid repetition']
      }
    };
  }
}