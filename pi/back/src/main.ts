...
console.log('  - PORT from Render:', process.env.PORT || '3000 (default)');
...
async function bootstrap() {
  ...
  app.use(
    session({
      secret: process.env.SESSION_SECRET || process.env.JWT_SECRET || 'dev-session-secret-change-me',
      resave: false,
      saveUninitialized: false,
      cookie: {
        maxAge: 15 * 60 * 1000,
  ...
  const frontendUrl = process.env.FRONTEND_URL || 'https://evalai.onrender.com';
  const isProduction = process.env.NODE_ENV === 'production';

  app.enableCors({
    origin: [
      frontendUrl,
      'https://evalai.onrender.com',  // Your frontend
  ...
  // IMPORTANT: Use the PORT from Render's environment
  ...
  console.log(`🌐 CORS enabled for: ${frontendUrl}`);
  console.log(`🔗 Health check: https://pi-backend-k23t.onrender.com/health`);
}
